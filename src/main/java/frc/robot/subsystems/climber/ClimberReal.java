...
public class ClimberReal implements ClimberIO {
    ...
    // private SparkClosedLoopController leftPIDController, rightPIDController;
    ...
    public ClimberReal() {
        ...
        config.softLimit
            .reverseSoftLimit(ClimberConstants.MIN_HEIGHT + .01)
            .reverseSoftLimitEnabled(true)
            .forwardSoftLimit(ClimberConstants.MAX_HEIGHT - .01)
            .forwardSoftLimitEnabled(true);
        config.encoder
                .positionConversionFactor(Constants.ClimberConstants.ENCODER_VELOCITY_MPS); // convert to radians
        config.closedLoop
                ...
                // .outputRange(0, 1);
        leftMotorController = new SparkMax(ShooterConstants.SHOOT_LOW_MOTOR_ID, MotorType.kBrushless);
        leftMotorController.configure(config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
        rightMotorController = new SparkMax(ShooterConstants.SHOOT_LOW_MOTOR_ID, MotorType.kBrushless);
        config.inverted(true);
        rightMotorController.configure(config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
        ...
        // leftPIDController = leftMotorController.getClosedLoopController();
        // rightPIDController = rightMotorController.getClosedLoopController();
